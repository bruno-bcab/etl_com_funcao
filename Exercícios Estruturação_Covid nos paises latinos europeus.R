...
latin_america <- coviddata %>% filter(location %in% latin_america_countries) # filtra casos apenas no vetor
...
general_data<-fread("https://docs.google.com/spreadsheets/d/e/2PACX-1vQ7EhiAkgUkjnBSJEsXn5qlvxZwwxF1uHdfuBa9X7uwdH9aTihZtnVcid8s1jGHtw/pubhtml?gid=1417881037&single=true") # carrega dados de DATARSU PERNAMBUCO
...
Exercício Países latinos na EUROPA

general_data<-fread("https://covid.ourworldindata.org/data/owid-covid-data.csv") # carrega dados de covid19 no mundo

latin_europe_countries<-c("Portugal", "Spain", "French", "Italy") # vetor que identifica países latinos na Europa

latin_europe<- general_data %>% filter(location %in% latin_europe_countries) # filtra casos apenas no vetor

mlatin <- latin_europe %>% group_by(location) %>% mutate(row = row_number()) %>% select(location, new_cases, row) # cria matriz dos países, agrupando por local, criando uma nova linha com index e selecionando apenas algumas variáveis

# filtra dados para garantir que todos os países tenham mesmo número de casos

result <- mlatin %>% group_by(location) %>% filter(row == max(row))

mlatin <- mlatin %>% filter(row<=min(result$row)) 

# pivota o data frame de long para wide nos países latinos europeus no frame covid no mundo
