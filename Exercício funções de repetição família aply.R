...
apply(DATARSU[ ,-1], 2, mean) # retira a primeira coluna que não é numérica
lapply(DATARSU[, -1], mean)
sapply(DATARSU[, -1], mean)
par(mfrow = c(3, 3))
sapply(DATARSU[ ,1:5], histDATA)
hist(DATARSU)
  
install.packages("HistData")
library(HistData)
apply(DISTRSU[ ,-1], 2, mean)
lapply(DISTRSU[, -1], mean)
sapply(DISTRSU[, -1], sum)
par(mfrow = c(2, 2))
sapply(DISTRSU[ , 1:4], hist)
